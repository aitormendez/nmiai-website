<div class="{{ $block->classes }} not-prose">
  <a href="{{ $person_link['people_link_link'] }}" class="wrapper relative block h-full w-full">
    ...
    @if ($person_link['people_link_content_type'] === 'video')
      <video poster="{{ $person_link['people_link_video_poster'] }}" autoplay muted loop class="relative">
        <source
          src="https://{{ $person_link['people_link_video_zone'] }}.b-cdn.net/{{ $person_link['people_link_video_id'] }}/play_720p.mp4"
          type="video/mp4">
      </video>
    @else
    ...
    <div class="bg-midup absolute left-0 bottom-0 flex items-center py-1 px-2 text-white">
      ...
      {{ $person_link['people_link_short_name'] }}
    </div>
  </a>
</div>
